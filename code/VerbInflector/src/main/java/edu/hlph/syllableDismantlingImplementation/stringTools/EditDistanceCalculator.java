...
public class EditDistanceCalculator {
    ...
    public int calc(String x, String y) {
        ...
        for (int i = 0; i < Math.max(xLen, yLen); i++) {  // initialization of the distance matrix
            if (i < xLen) {
        ...
        for (int i = 1; i < xLen; i++) {
            for (int j = 1; j < yLen; j++) {
                this._distMat[i][j] = min3(
                        this._distMat[i - 1][j - 1] + this.delta(x.substring(i - 1, i), y.substring(j - 1, j)),
    ...
    protected int delta(String a, String b) {
        return a.equals(b) ? this._wCopy : this._wReplace;
    }
}
